...
module.exports = function(arrayBuffer) {
	...
	function read2(){
		var val = dataView.getUint16(position, false); // false表示大端序
		position+=2;
		return val;
	}
	function read4(){
		var val = dataView.getUint32(position, false); // false表示大端序
		position+=4;
		return val;
	}
	function read(length, enc){
		if(length==0) return '';
		// 读取字节并转换为字符串
		var bytes = new Uint8Array(arrayBuffer, position, length);
		position += length;
		...
		if (enc === 'ascii') {
			...
			// UTF-8解码
			var decoder = new TextDecoder(enc || encoding);
			return decoder.decode(bytes);
		}
	}
	function readGroups(){
		var group;
		while(position < arrayBuffer.byteLength && (group = read1()) !== 0x03){//end-of-attributes-tag
			readGroup(group);
		}
	}
	function readGroup(group){
		...
		while(dataView.getUint8(position) >= 0x0F) {// delimiters are between 0x00 to 0x0F
			readAttr(group);
		}
	}
	...
	if(position < arrayBuffer.byteLength) {
		// 读取剩余数据
		var remainingBytes = new Uint8Array(arrayBuffer, position);
		var decoder = new TextDecoder(encoding);
		obj.data = decoder.decode(remainingBytes);
	}
